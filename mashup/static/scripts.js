...
function addMarker(place)
{
    ...
    marker.addListener('click', function() {
        ...
        // Get JSON articles from server
        let parameters = {
            geo: place["postal_code"]
        };
        $.getJSON("/articles", parameters, function(data, textStatus, jqXHR) {

            // Create unordered list with data received
            let content = "<ul>";

            for(let x=0; x < data.length; x++)
            {
                content += "<li><a href=\"" + data[x]["link"] + "\"target=\"_blank\">" +
                    data[x]["title"] + "</a></li>";
            }
            content += "</ul>";
...
function removeMarkers()
{
    ...
    // Deletes references of all marker in array
    markers = [];
}
